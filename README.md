...
Koishi 是一个**纯本地**的 Android 工具箱。大部分工具处理都在设备端完成
少数工具会按需访问第三方公开接口，部分工具的 API Key 需要由用户自行填写并保存在本机。
...
| ⚖️ **BMI 计算** | 多次记录身高体重并绘制 BMI 变化曲线，分级区间着色，支持单条删除与一键清空 |

### 其他应用

| 工具 | 简介 |
| --- | --- |
| 📦 **轻应用** | 把常用网页收进应用，内置轻量 WebView 浏览与广告追踪拦截，支持长按拖动排序、手机 / 电脑模式切换，缓存、Cookie 与本地存储一键清理 |
